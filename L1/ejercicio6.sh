...
pasarela=$(snmpget -v1 -c gar tales 1.3.6.1.2.1.4.21.1.7.$1 2>/tmp/errorpasarela | awk '{print $4}')
texterror=$(cat /tmp/errorpasarela)
...
then
	...
	echo -e "${redColour}Pasarela: ${endColour}${purpleColour}$pasarela${endColour}"
fi
